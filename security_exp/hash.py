...
with tempfile.NamedTemporaryFile() as fa:
    with tempfile.NamedTemporaryFile() as fb:
        ...
        def get_hash(fp, text):
            fp.write(text)
            fp.seek(0)
            #result = os.popen("openssl dgst -sha256 {}".format(fp.name)).read()[66:-1]
            result = os.popen("openssl dgst -md5 {}".format(fp.name)).read()[65:-1]
            print result
            if result[0] == '=':
                result = result[2:]
            #print result
        ...
        print changed_hash
        #计算不同的位数
        print "same count:", reduce(lambda x, y: x+y, [origin_hash[i] == changed_hash[i] for i in range(len(origin_hash))])
